...
class VisualsUISubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		var option:Option = new Option('Hold Splash Opacity',
			'How much transparent should the Hold Splashes be.\nAffects the END of the splash, usually for the player\'s side.',
			'holdSplashAlpha',
			'percent');
		option.scrollSpeed = 1.6;
		option.minValue = 0.0;
		...
		var option:Option = new Option('Time Bar:',
			...
			'timeBarType',
			'string',
			['Time Left', 'Time Elapsed', 'Song Name', 'Disabled', 'Elapsed, Left']);
		addOption(option);
	...
	{
		if(changedMusic && !OptionsState.onPlayState) FlxG.sound.playMusic(Paths.music('menuSongs/freakyMenu-' + ClientPrefs.data.menuSong));
		super.destroy();
	}
